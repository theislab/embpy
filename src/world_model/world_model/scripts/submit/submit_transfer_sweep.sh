...
#
# Usage:
#   bash src/world_model/world_model/scripts/submit/submit_transfer_sweep.sh
...
EMBEDDINGS_DEFAULT="$(pixi run -e gpu -- python -m world_model.configs.run_matrix default-embeddings --target slurm-transfer)"
...
NADIG_H5AD="${NADIG_H5AD:-$(pixi run -e gpu -- python -m world_model.configs.run_matrix h5ad nadig)}"
REPL_H5AD="${REPLOGLE_H5AD:-$(pixi run -e gpu -- python -m world_model.configs.run_matrix h5ad replogle)}"
...
pixi_env_for() {  # $1=embedding -> echoes env name
    pixi run -e gpu -- python -m world_model.configs.run_matrix pixi-env "$1"
}
