...
```bash
pixi run -e gpu python -m world_model.scripts.smoke_test
```
...
```bash
pixi run -e gpu python -m pytest -x tests/world_model/test_dataset.py
pixi run -e gpu python -m pytest -x tests/world_model/test_splits.py
```

Expected: every test passes. Sanity-check a real dataset interactively:

```python
from world_model.configs import DataConfig, SplitConfig
...
```bash
pixi run -e gpu python -m pytest -x tests/world_model/test_state_stack_encoder.py
```
...
```bash
pixi run -e gpu python -m pytest -x tests/world_model/test_gpt_autoregressive.py
```
...
```bash
pixi run -e gpu python -m pytest -x tests/world_model/test_losses.py
pixi run -e gpu python -m pytest -x tests/world_model/test_world_model.py
```
...
```bash
pixi run -e gpu python -m pytest -x tests/world_model/test_hooks.py
```
...
```bash
pixi run -e gpu python -m pytest -x tests/world_model/test_baselines.py
```
...
```bash
pixi run -e gpu python -m pytest -x tests/world_model/test_metrics.py
```
...
```bash
pixi run -e gpu python -m pytest -x tests/world_model/test_prep.py
pixi run -e gpu python -m pytest -x tests/world_model/test_plots.py
```
...
```bash
pixi run -e gpu pytest -k adapter_factory_parity tests/world_model
```
...
from world_model.models.action.adapters import LinearAdapter, MLPAdapter, LoRAAdapter
adapters = [
...
`provider._load()`, so `world_model` is fully importable on a
...
pixi run -e gpu pytest tests/world_model/test_local_backbone_parity.py -k local_backbone_parity -v
