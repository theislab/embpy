...
# --- depends on embpy: ---
# Every cross-package import into `embpy` is listed here so the boundary
# between `world_model` (perturbation modelling) and `embpy`
# (infrastructure: embeddings, annotations, resolvers) is visible at a
# glance. Keep this list in sync with grep -RIn "^from embpy\." src/world_model.
#
#   from embpy.resources.gene.control import ControlPolicy
#       data/embeddings/{registry,bio_embedder,precomputed}.py
#       scripts/embed_perturbations.py
#   from embpy.embedder import BioEmbedder            (lazy, _get_embedder)
#       data/embeddings/bio_embedder.py
#   from embpy.models.singlecell_models import StackWrapper, StateEmbeddingWrapper  (lazy)
#       models/encoders/backbones/{stack,state}.py
#
# Direction is one-way: embpy must not import world_model. The boundary
# is enforced by tests/embpy/test_boundary.py.
# --- end depends on embpy ---
