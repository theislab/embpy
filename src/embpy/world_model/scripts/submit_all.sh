...
ablate_action_encoder() {
    local base_config="src/embpy/world_model/configs/experiments/single_replogle.yaml"
    local grid="src/embpy/world_model/configs/experiments/ablation_action_encoder.yaml"
...
import sys
from embpy.world_model.evaluation.ablation import load_grid
...
lone() {
    local base_config="src/embpy/world_model/configs/experiments/transfer.yaml"
    local grid="src/embpy/world_model/configs/experiments/ablation_action_encoder.yaml"
    ...
    agg_jid=$(sbatch --parsable \
        ...
        --wrap="set -euo pipefail; \
            ...
            pixi run -e gpu -- python -c \
                'from embpy.world_model.scripts.leave_one_encoder_out import _summary_long_for_strategy, _render_strategy_heatmaps; \
                 from embpy.world_model.evaluation.ablation import resolve_grid; \
